...
/**
 * Check that there is no unexpected field in the body
 * Set a global error if there are any
 * @params body
 * @private
 */
Form.prototype._checkUnknownFields = function (body) {
    for (var fieldName in body) {
        if (!(fieldName in this._oFields)) {
            this.addGlobalError(Errors._unknownField);
            break;
        }
    }
};
...
Form.prototype.validate = function () {
    ...
    vForm._checkUnknownFields(body);
        ...
        return new Promise(function (resolve, reject) {
            self._validateCallback(vForm, body, function (vForm) {
                if (vForm.isValid()) {
                    ...
                    reject(new Form.ValidationError(vForm));
                }
            });
        });
    }
 ...
 * @param {?string|Function}  error - The error message
 * @static
 */
