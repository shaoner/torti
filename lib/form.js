 ...
 * @public
 */
Form.prototype.validate = function (body, stop) {
    body = body || { };
    stop = stop || false;
    var bval, f, newVal;
    ...
    for (var fieldName in body) {
        ...
        if (!field) {
            renderForm.valid = false;
            renderForm.globalError = Form.validators._unknownField;
    ...
    for (var i = 0, len = fields.length; i < len; ++i) {
        ...
        renderForm.setField(f._name, 'value', newVal.value);
        if (newVal.error) {
            renderForm.setField(f._name, 'error', newVal.error);
            renderForm.valid = false;
            if (stop === true) { break; }
        }
    }
